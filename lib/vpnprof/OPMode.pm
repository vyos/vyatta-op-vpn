...
sub get_tunnel_info_profile {
  my $profile = pop(@_);
  my @tunnels = split(' ', `cli-shell-api listActiveNodes vpn ipsec profile $profile bind tunnel`);
  ...
  my $first_addr = 1;
  for my $tun (@tunnels) {    
    my @addresses = split(' ', `cli-shell-api returnActiveValues interfaces tunnel $tun address`);
...
sub process_tunnels{
  ...
  foreach my $line (@ipsecstatus) {
    if (($line =~ /\"(vpnprof-tunnel-.*?)\"/) || ($line =~ /\"(.*-to-.*)\"/)) {
      my $connectid = $1;
	...
	$line =~ /\"(.*)-to-(.*)\"/;
        ...
        my @tunnels = split(' ', `cli-shell-api listActiveNodes interfaces tunnel`);
        for my $tun (@tunnels) {
          my @addresses = split(' ', `cli-shell-api returnActiveValues interfaces tunnel $tun address`);
          for my $addr (@addresses) {
...
{
  my @profiles = split(' ', `cli-shell-api listActiveNodes vpn ipsec profile`);
